...
class NodeModules {
  ...
  has (dependency) {
    const exists = this.allDependencies.some(({modulePath}) => modulePath === dependency.modulePath)
    return exists
  }
  ...
  // scoped module を展開する
  async _extractScoped (names, parent = null) {
    let extracted = []
    for (const name of names) {
      if (name.startsWith('@')) {
        const path = [this.projectDir, parent && parent.modulePath, 'node_modules', name].filter(Boolean)
        const modulePath = join(...path)
        const scopedNames = (await readdirAsync(modulePath))
          .map((mod) => `${name}/${mod}`)
  ...
  async _childenOf (dependency) {
    ...
    if (hasChildren) {
      ...
      names = await this._extractScoped(names, dependency)
